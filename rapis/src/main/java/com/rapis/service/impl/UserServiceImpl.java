...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Result<String> getUser(HttpServletRequest httpServletRequest) {
        ...
        for(Cookie cookie :cookies){
            if(cookieTokenName.equals(cookie.getName())) {
                ...
                if(redisTemplate.hasKey(redisKey)!=null&&redisTemplate.getExpire(redisKey)>0)
                    return Result.result(CodeMsg.UTIL_SUCCESS,redisTemplate.opsForValue().get(redisKey).toString());
            }
        }
        return Result.result(CodeMsg.LOGIN_NOT);
    }
